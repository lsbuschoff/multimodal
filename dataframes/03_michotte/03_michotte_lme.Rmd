...
setwd("~/Documents/Projects/multimodal/dataframes/03_michotte")
```
...
for (ind in seq_along(subjects)) {
  ...
  temp <- read.csv(str_glue("{subjects[[ind]]}_outcome.csv"))
...
# Write
write.table(out, file="03_outcome_lmer.csv", sep = ",", row.names = TRUE, col.names = TRUE)
```
...
for (ind in seq_along(subjects)) {
  ...
  if (sum(!is.na(unique(temp$Model))) > 1) {
    ...
    # Multiply with sign of coefficient again 
    coefsign = sign(fixef(temp.lm)[2, 1])
    out[ind, 1] = coefsign * sqrt(abs(R2[1]))
    
    # If positive then order of CIs is correct
    if (coefsign == 1) {
      out[ind, 2] = coefsign * sqrt(abs(R2[3]))
      out[ind, 3] = coefsign * sqrt(abs(R2[4]))
      
    # If negative, CIs have to be flipped
    } else {
      out[ind, 3] = coefsign * sqrt(abs(R2[3]))
      out[ind, 2] = coefsign * sqrt(abs(R2[4]))
    }
  }
}

# Write
write.table(out, file="03_counterfactual_lmer.csv", sep = ",", row.names = TRUE, col.names = TRUE)
```
