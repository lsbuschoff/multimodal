...
parser.add_argument('--model', type=str, default="OTTER")
...
# model = OtterForConditionalGeneration.from_pretrained("luodian/OTTER-9B-DenseCaption", device_map="sequential", **precision)
...
# Open the CSV file and create csv writer
            with open(f"{model_str}/results.csv", 'w', newline='') as csvfile:
                writer = csv.writer(csvfile)
                    
                # Set directory and get list of all images
                directory = "images/lerer100"
                ...
                for i, seq in enumerate(images):
                    ...
                    if exp == 0:
                        ...
                        instruction = "Q: What are the colors of the blocks from top to bottom? You are only allowed to answer with the color names." 
                    ...
                    writer.writerow([seq, instruction, response])
        ...
        # For Michotte stimuli
            ...
            with open(f"{model_str}/results.csv", 'w', newline='') as csvfile:
                ...
                for i, seq in enumerate(images):
                    ...
                    # Set prompts for relevant experiment
                    if exp == 0:
                        instruction = "Q: The scene shows two balls labeled “A”, and “B”. On the left side there is a pink gate. The solid arrows show the trajectories of the balls. What is the background color of the image? You are only allowed to answer with a single color name!" 

                    elif exp == 1:
                            instruction = "Q: The scene shows two balls labeled “A”, and “B”. On the left side there is a pink gate. The solid arrows show the trajectories of the balls. Do the balls move “left to right” or “right to left”? You are only allowed to respond with one of the two options!"
                    
                    elif exp == 2:
                        if i < 7 or i == 8 or i == 10:    
                            instruction = "Q: The scene shows two balls labeled “A”, and “B”. On the left side there is a pink gate. The solid arrows show the trajectories of the balls. Indicate your agreement with the assessment that ball “B” completely missed the gate on a scale from 0 (not at all) to 100 (very much). You are only allowed to answer with a single number."

                        else:
                            instruction = "Q: The scene shows two balls labeled “A”, and “B”. On the left side there is a pink gate. The solid arrows show the trajectories of the balls. Indicate your agreement with the assessment that ball “B” went through the middle of the gate on a scale from 0 (not at all) to 100 (very much). You are only allowed to answer with a single number."
                    ...
                    # Model output
                    response = helpers.get_response(image, instruction, model, image_processor, max_tokens=7)
                    prompts += (f'{response}\n')

                    # Write a row to the CSV file
                    writer.writerow([seq, instruction, response])

        # For Gerstenberg agents stimuli
        elif args.dataset == "AGENTS":
            ...
            with open(f"{model_str}/results.csv", 'w', newline='') as csvfile:
                ...
                for exp in range(4):
                    ...
                    for i, seq in enumerate(images):
                        ...
                        # Set prompt for background question
                        if exp == 0:
                            instruction = "Q: What is the background color of the image? You are only allowed to answer with a single color name!" 
    
                         # Set prompt for counting question
                        elif exp == 1:
                                instruction = "Q: How many boxes are in the scene? You are only allowed to respond with a single number."
    
                        # Set prompt for intention question
                        elif exp == 2:
                                instruction = "Q: What was BLUE intending to do? Give your answer on a scale from “definitely hinder RED” (0) to “definitely help RED” (100) with the midpoint “unsure” (50). You are only allowed to respond with a single number."
    
                        # Set prompt for counterfactual question
                        elif exp == 3:
                            # Failure trials 
                            if i < 7 or i == 9 or i == 10 or i == 13 or i == 14 or i == 15: 
                                instruction = "Q: How much do you agree that RED would have succeeded if BLUE hadn't been there on a scale from “not at all” (0) to “very much” (100)?. You are only allowed to answer with a single number."
                            # Success trials
                            else:
                                instruction = "Q: How much do you agree that RED would still have succeeded if BLUE hadn't been there on a scale from “not at all” (0) to “very much” (100)?. You are only allowed to answer with a single number."

                        # Make output
                        comb_instruction = basic_prompt + instruction
                        prompts += (f'\nEXP_{exp+1}, {seq}: ')
                        image = helpers.get_image(seq)
                        image = image.convert("RGB")
    
                        # Model output
                        response = helpers.get_response(image, comb_instruction, model, image_processor)#, max_tokens=7)
                        prompts += (f'{response}\n')
        ...
        # For Jara-Ettinger Naive Utility Calculus (NUC)
            ...
            for i, seq in enumerate(images):
                ...
                for instruction in instructions:
                    ...
                    # Model output
                    response = helpers.get_response(image, comb_instruction, model, image_processor)#, max_tokens=1)
                    prompts += (f'{response}\n')
